...
* 🎁 ⚙️ **[re-think_v1_pseudo_code.md](prompts/re-think_v1_pseudo_code.md)** (伪代码版 / Production Core 隐藏彩蛋)
  * **适用对象：** 硬核生产环境部署 (Hardcore production)、RAG 管道、多智能体编排器。
  * **特点：** 这是为架构师准备的专属机制。完全使用英语伪代码、集合论和逻辑运算符编写。Token 消耗量与英文精简版大致相当（~1,300 tokens），但由于数学运算符和变量名不存在歧义空间，该版本在长对话中对"注意力漂移"（Attention Drift）表现出显著更强的抵抗力。它以高密度语义指令的形式直接作用于模型的推理权重，提供最高的语义密度，无任何语言冗余。
